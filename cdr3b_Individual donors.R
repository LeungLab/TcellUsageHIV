...
#For the expanded clones we find the distribution of CDR3b sequences as well as the length of the CDR3a sequences
...
final800<-final700[rep(row.names(final700), final700$value), 1:3]#Every CDR3b sequence that happens more then once gets a new row

final800$cdr3blength<-nchar(final800$CDR3b)#Add a new column which calculates the length of CDR3b

test100<- count(final800, Subject, Stimulation,cdr3blength)#This gives how many CDR3b are of a particular length for a given condition

write.csv(final800, "List of CDR3b_Individual donors.csv")#Gives the required output in csv form
write.csv(test100, "CDR3b length for individual donors.csv")#Gives the required output in csv form
